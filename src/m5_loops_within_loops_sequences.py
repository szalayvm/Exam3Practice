...
def run_test_integers():
    ...
    # Test 2:
    expected = [3, 1, 4, 10, 10, 1, 3, 4, 30, -4, 3, 3]
    answer = integers([(3, 1, 4),
                       (10, 'hi', 10),
                       [1, 2.5, 3, 4],
                       'hello','hello',
...
def big_letters(sequence_of_sequences):
    ...
    for k in range(len(sequence_of_sequences)):
        if type(sequence_of_sequences[k])== str:
            other_list.append(sequence_of_sequences[k])
        for j in range(len(sequence_of_sequences[k])):
           if type(sequence_of_sequences[k][j]) == str:
               new_list.append(sequence_of_sequences[k][j])
            
    for h in range(len(other_list)):
        for g in range(len(other_list[h])):
            if other_list[h][g].isupper()==True:
                new_string = new_string + other_list[h][g]
    return new_string
